...
<html lang="en">
  ...
  <?php include 'config.php'; ?>
			...
			<?php
				if(isset($_COOKIE['userrole']) )
				{
					echo '
						...
						<li class="nav-item px-lg-2">
						  <a class="nav-link text-uppercase text-expanded" href="view_topics">View Topics
						  </a>
						</li>
						<li class="nav-item px-lg-2">
	...
	<?php
		$q = mysqli_query($conn, "SELECT t.name topic, d.name department FROM library.topic t INNER JOIN library.department d on t.dept_no= d.dept_no" ) ;
		...
		if($check_user>0){
			...
			{	
				$url= "/library/search.php?searchby=topic&searchname=".$row["topic"];
				echo "
				<div class=\"row\">
					<div class=\"col-md-6\">
