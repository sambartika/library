  ...
  <?php include 'config.php'; ?>
			...
			<?php
				if(isset($_COOKIE['userrole']) )
				{
					...
					if($_COOKIE['userrole']=="admin")
					{
						...
						<li class="nav-item px-lg-2">
						  <a class="nav-link text-uppercase text-expanded" href="insert_topic">
						  Add Topic</a>
						</li>
						<li class="nav-item px-lg-2">
	...
	<?php
		...
		if (isSet($_POST['t_name']) && isSet($_POST['d_name']) && $_POST['t_name'] != '') 
		{
			...
			$q1 = mysqli_query($conn, "SELECT * FROM library.topic WHERE name='$topic_name'" ) ;
			$check_topic = mysqli_num_rows($q1);
			
			if($check_topic<1){
				$q2 = mysqli_query($conn, "SELECT * FROM library.topic where topic_id= (select max(topic_id) from library.topic)" ) ;
				...
				$sp[1]++;
				$ss= $ss.$sp[1];
			    $q = mysqli_query($conn, "INSERT INTO library.topic (topic_id, name, dept_no, shelf_no) VALUES ($topic_id, '$topic_name', $dept_no, '$ss');" ) ;
				$flag=3;
          ...
          <div class="row">
            ...
            <div class="form-group col-lg-4">
              <label class="text-heading">Department Name</label>
				<select name="d_name">
					<?php 
					$sql = mysqli_query($conn, "SELECT * FROM library.department");
