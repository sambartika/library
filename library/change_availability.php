...
<html lang="en" >
  ...
  <div class="tagline-upper text-center text-heading text-shadow text-white mt-5 d-none d-lg-block">Texas A&M University Library</div>
			...
			<?php
				if(isset($_COOKIE['userrole']) )
				{
					...
					if($_COOKIE['userrole']=="admin")
					{
						echo '
						<li class="nav-item px-lg-2">
						  <a class="nav-link text-uppercase text-expanded" href="change_availability">Change Availability
						  </a>
						</li>
						<li class="nav-item px-lg-2">
	...
	<?php
		...
		//Check if we need to search in database or update the database
		if(isSet($_POST['availupdate']) )
		{
			$q = mysqli_query($conn, "UPDATE library.book set availability=\"".$_POST['availability']."\" where book_id= ".$_POST['count'] ) ;
			$flag=3;
		}
		else{
		}
	...
	<div class="bg-faded p-2 my-2">
        ...
        <hr class="divider">
        <form name="search_by_name" accept-charset="utf-8" method="post" action="change_availability.php">
		<?php
		  ...
		  <?php
			...
			elseif($flag==3)
			{
				echo'
					...
					<div class="form-group col-lg-4">
						<label class="text-heading" style="color:red">Updated successfully</label>
					</div>
					<div class="form-group col-lg-4">
					</div>';
			}
	...
	{
		?>
		<form name="updateavailability" accept-charset="utf-8" method="post" action="change_availability.php">
	...
	echo "
				...
				<p>Available</p></br>
					...
					echo "</select>
					<button name=\"availupdate\" type=\"submit\" class=\"btn btn-secondary\">Enter</button>
				</div>
			</div>
			<hr class=\"divider\">
			";
