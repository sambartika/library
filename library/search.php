		...
		if ((isSet($_POST['searchname']) && isSet($_POST['searchby'])) || ((isSet($_GET['searchname']) && isSet($_GET['searchby'])))) 
		{
			...
			if(strcmp($searchby, "name")==0 || strcmp($searchby, "author")==0)
			{
				$q = mysqli_query($conn, "SELECT max(b.book_id) book_id, max(b.name) name,max(b.author)author, max(b.description) description,
										  ...
										  FROM book b 
										  INNER JOIN topic_of_book tb ON b.book_id=tb.book_id
										  INNER JOIN topic t ON tb.topic_id= t.topic_id
          ...
          $q= mysqli_query($conn, "select * from book where $searchby like '%$searchname%'");
          ...
          if($cc>0)
          {
			...
			if($flag== 1)
			{
				while ($row = mysqli_fetch_assoc($q)) 
				{
					...
					$location= "D.FL".$row["lib_floor"].".R".$sp[0].".".$row["location"];
					$books[$i]= array(
						"book_id" => $book_id,
						"book_name" => $book_name,
