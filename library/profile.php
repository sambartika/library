  ...
  <?php include 'config.php'; ?>
			...
			<?php
				if(isset($_COOKIE['userrole']) )
				{
					echo '
						...
						<li class="nav-item px-lg-2">
						  <a class="nav-link text-uppercase text-expanded" href="profile">Update Profile
						  </a>
						</li>';
	...
	<?php
		...
		{	
			...
			if(mysqli_query($conn, "UPDATE library.member set first_name=\"".$fname1."\", last_name= \"".$lname1."\", email= \"".$email1."\",
		...
		$q = mysqli_query($conn, "SELECT * from library.member where email=\"".$_COOKIE['username']."\"" ) ;
