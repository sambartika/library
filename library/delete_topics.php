...
<html lang="en" >
  ...
  <?php include 'config.php'; ?>
			...
			<?php
				if(isset($_COOKIE['userrole']) )
				{
					...
					if($_COOKIE['userrole']=="admin")
					{
						...
						<li class="nav-item px-lg-2">
						  <a class="nav-link text-uppercase text-expanded" href="delete_topics">Remove Topic
						  </a>
						</li>';
					}
	...
	<?php
		...
		{
			$q = mysqli_query($conn, "DELETE FROM library.topic where topic_id= ".$_POST['count'] ) ;
			$q = mysqli_query($conn, "DELETE FROM library.book_of_topic where topic_id= ".$_POST['count'] ) ;
		}

		if(isSet($_POST['searchname']) && $_POST['searchname']!= "" )
		{
			$q = mysqli_query($conn, "SELECT * from library.topic where name like \"%".$_POST['searchname']."%\" limit 1" ) ;
